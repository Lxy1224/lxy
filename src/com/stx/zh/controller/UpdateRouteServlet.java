		 ...
		 DiskFileItemFactory factory = new DiskFileItemFactory();
	        ...
	        try {
	            ...
	            for (FileItem item : items) {
	                if (item.isFormField()){
	                    //普通表单
	                    if ("r_no".equals(item.getFieldName())){
	                    	route.setR_no(Integer.parseInt(item.getString("utf-8")));
	                    }
	                    if ("r_label".equals(item.getFieldName())){
	                    	route.setR_label(item.getString("utf-8"));
	                    }
	                    if ("r_title".equals(item.getFieldName())){
	                    	route.setR_title(item.getString("utf-8"));
	                    }
	                    if ("r_content".equals(item.getFieldName())){
	                    	route.setR_content(item.getString("utf-8"));
	                    }
	        ...
	        request.getRequestDispatcher("/AllRouteServlet").forward(request,response);
	}
